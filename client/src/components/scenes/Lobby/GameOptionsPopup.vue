  ...
  <div class="game-options-popup" @click.self="close">
    <div class="game-options-popup__wrapper">
      ...
      <div class="game-options-popup__content">
        <Checkbox v-for="pack in gameSettings.packs" :checkbox="pack" />
      </div>
      <h3 class="game-options-popup__title">{{ $text('GAME_ROOMS.ADDITIONAL_OPTIONS') }}</h3>
      <div class="game-options-popup__content">
        <Checkbox v-for="option in gameSettings.options" :checkbox="option" />
      </div>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        gameSettings: Object.assign({}, this.settings),
      };
    },
